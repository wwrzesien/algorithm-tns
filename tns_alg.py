...
class ALgorithmTNS(object):
        ...
        for item_i in range(self.db.min_item, self.db.max_item + 1):
            ...
            for item_j in range(item_i + 1, self.db.max_item + 1):
                ...
                if sup_ji >= self.min_supp_relative:
                    ...
                    rule_ji = Rule()
                    rule_ji.rule([item_j], [item_i], conf_ji, sup_ji, tids_j, tids_i, occurrences_j_first, occurrences_i_last)
    ...
    def save(self, rule, support):
        ...
        while lower_rule_node != None and lower_rule_node.item != None and lower_rule_node.get_absolute_support() == support:
            # Strategy 1:
            # if the confidence is the same and the rule "lower_rule_node" subsume the new rule
            # then we dont add the new rule
            if rule.confidence == lower_rule_node.confidence and self.subsume(lower_rule_node.item, rule):
                self.not_added += 1
                return
    ...
    def register_as_candidate(expand_lr, rule_lr):
        ...
        for tid in rule.tids_ij:
            ...
            for k in range(0, end):
                ...
                for item in itemset:
                    ...
                    if self.containsLEX(rule.itemset1, item) or self.containsLEX(rule.itemset2, item):
                        continue
                    ...
                    if item not in frequnet_items_c.keys():
